...
<template>
    <div class="fon">
        <div  class="productsINcatalog">
            <div class="products" v-for="(product,index) in this.pendant" :key="product.model">
...
export default {
    ...
    mounted() {
        for (let i=0;i<this.$store.state.products.length;i++){
            if(this.$store.state.products[i].category==="pendant"){
                this.pendant.push(this.$store.state.products[i])
            }
        }
    },
